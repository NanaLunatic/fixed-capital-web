...
// 搜索表格hooks操作
const { querySearch, options, searchData, tableData } = useFetchSearch({ api: searchDelivery, options: localOptions })
...
const handelExport = () => {
  ...
  exportLogs(deliveryDownload, formData, "出库记录")
}
    ...
    <div v-loading="loading" shadow="never" class="flex-fill">
      <div class="menu-op">
        <div class="content">
          <el-button
            type="warning"
            v-show="buttonVisibility('outbound:export')"
            plain
            :icon="Download"
            @click="handelExport"
            >导出</el-button
