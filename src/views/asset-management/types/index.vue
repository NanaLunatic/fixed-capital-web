...
import { searchTypes, typesDownload, deleteTypes } from "@/api/assetManagement/types/index"
...
// 搜索表格hooks操作
const { querySearch, options, searchData, tableData } = useFetchSearch({ api: searchTypes, options: localOptions})
...
const handelExport = () => {
  ...
  exportLogs(typesDownload, formData)
}
...
const actions = ref([
  {
    ...
    condition: (row: any) => buttonVisibility('types:edit'),
  }
])
...
<template>
  <div class="main-content">
    ...
    <el-card v-loading="loading" shadow="never" class="flex-fill">
      <div class="menu-op">
        <div class="content">
          <el-button type="primary" v-show="buttonVisibility('types:add')" plain :icon="CirclePlus" @click="handelFun('add')">新增</el-button>
          <el-button type="danger" v-show="buttonVisibility('types:delete')" plain :icon="Delete" @click="handelDel">删除</el-button>
          <el-button type="warning" v-show="buttonVisibility('types:export')" plain :icon="Download" @click="handelExport">导出</el-button>
        </div>
      </div>
      <PublicTable
        ref="publicTableRef"
