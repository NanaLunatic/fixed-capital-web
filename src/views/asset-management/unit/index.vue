...
import { searchUnits, unitsDownload, deleteUnits } from "@/api/assetManagement/unit/index"
...
// 搜索表格hooks操作
const { querySearch, options, searchData, tableData } = useFetchSearch({ api: searchUnits, options: localOptions})
...
const handelExport = () => {
  ...
  exportLogs(unitsDownload, formData)
}
...
const actions = ref([
  {
    ...
    condition: (row: any) => buttonVisibility('unit:edit'),
  }
])
...
<template>
  <div class="main-content">
    ...
    <el-card v-loading="loading" shadow="never" class="flex-fill">
      <div class="menu-op">
        <div class="content">
          <el-button type="primary" v-show="buttonVisibility('unit:add')" plain :icon="CirclePlus" @click="handelFun('add')">新增</el-button>
          <el-button type="danger" v-show="buttonVisibility('unit:delete')" plain :icon="Delete" @click="handelDel">删除</el-button>
          <el-button type="warning" v-show="buttonVisibility('unit:export')" plain :icon="Download" @click="handelExport">导出</el-button>
        </div>
      </div>
      <PublicTable
        ref="publicTableRef"
